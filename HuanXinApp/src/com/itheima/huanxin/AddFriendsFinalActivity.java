...
public class AddFriendsFinalActivity extends BaseActivity{
    ...
    public void addContact(final String glufine_id,final String myreason) {
        ...
        if (MyApplication.getInstance().getContactList().containsKey(glufine_id)) {
        	T.show(this, "此用户已是你的好友");
            return;
        }
        ...
        new Thread(new Runnable() {
            public void run() {
                try {
                    ...
                    String name = LocalUserInfo.getInstance(AddFriendsFinalActivity.this).getUserInfo("nick");
                    String avatar = LocalUserInfo.getInstance(AddFriendsFinalActivity.this).getUserInfo("avatar");
                    long time = System.currentTimeMillis();
                    String myreason_temp=myreason;
                    if(myreason==null||myreason.equals("")){
                        myreason_temp = "请求加你为好友";
                    }
                    String reason = name + "66split88" + avatar + "66split88"+ String.valueOf(time)+"66split88"+myreason_temp;
                    EMContactManager.getInstance().addContact(glufine_id,reason);
